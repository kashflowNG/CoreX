    ...
    for (const user of allUsers) {
      ...
      if (user.currentPlanId) {
        ...
        if (plan && plan.isActive) {
          ...
          intervalRate = dailyRate / 144;
          planName = plan.name;
        }
      } else {
        // Tiny increase rate for free plans (example: 0.0001 daily)
        intervalRate = 0.00000069444444; // 0.0001 / 144
      }
...
Plan: ${planName}
Rate: ${(intervalRate * 100).toFixed(6)}% per interval
...
Your new balance: ${newBalance.toFixed(8)} BTC`,
        ...
        console.log(`Updated balance for user ${user.id}: +${increase.toFixed(8)} BTC (${planName})`);
      }
    }
  } catch (error) {
